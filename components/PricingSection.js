...
const plans = [
  {
    id: 'free', name: 'Starter', monthlyPrice: 0, yearlyPrice: 0, color: '#6b7280',
    description: 'Perfect for exploring AI tools',
    features: ['5 AI prompts per day','3 prompt categories','Full AI tools directory','Free tools access','Ad-supported'],
    limits: ['No prompt saving','No video/coding/audio prompts'],
    cta: 'Get started free', popular: false,
  },
  {
    id: 'pro', name: 'Pro', monthlyPrice: 9, yearlyPrice: 7, color: '#6366f1',
    description: 'For AI power users and creators',
    features: ['Unlimited prompts daily','All 8 prompt categories','Save & export prompts','Prompt history (30 days)','No ads ever','Priority support'],
    limits: [], cta: 'Start Pro', popular: true,
  },
  {
    id: 'team', name: 'Team', monthlyPrice: 29, yearlyPrice: 23, color: '#7c3aed',
    description: 'For teams building with AI',
    features: ['Everything in Pro','Up to 10 team members','Shared prompt library','Prompt history (1 year)','Team analytics','Dedicated support'],
    limits: [], cta: 'Start Team', popular: false,
  },
];
...
export default function PricingSection({ id = 'pricing' }) {
  ...
  const getButtonState = (plan) => {
    if (plan.id === 'free') {
      if (!user) return { label: plan.cta, disabled: false };
      return { label: '✓ Current plan', disabled: true };
    }
    if (profile?.plan === plan.id) return { label: '✓ Current Plan', disabled: true };
    if (loadingPlan === plan.id) return { label: 'Loading...', disabled: true };
    ...
    <section id={id} className="py-20 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl font-black text-gray-900 mb-3">Start free. Upgrade when ready.</h2>
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            ...
            <button onClick={() => setBilling('yearly')}
              ...
              Yearly
              <span className={`text-xs px-2 py-0.5 rounded-full font-black ${billing==='yearly' ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'}`}>Save 22%</span>
            </button>
          </div>
        </div>
        ...
        {error && (
          <div className="mb-6 bg-red-50 border border-red-200 rounded-2xl p-4 text-red-700 text-sm text-center">
            ...
            {error.includes('log in') && (
              <button onClick={() => router.push('/login?redirect=' + encodeURIComponent('/#pricing'))}
                className="ml-2 underline font-bold">Log in now →</button>
            )}
          </div>
        )}
        ...
        {!user && (
          <div className="mb-8 bg-indigo-50 border border-indigo-200 rounded-2xl p-4 text-center text-sm text-indigo-700">
            <span className="font-semibold">Create a free account</span> to upgrade to Pro or Team.{' '}
            <button onClick={() => router.push('/login?redirect=' + encodeURIComponent('/#pricing'))}
              className="underline font-bold">Sign up free →</button>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map(plan => {
            ...
            const isCurrentPlan = profile?.plan === plan.id || (!profile && plan.id === 'free');

            return (
              <div key={plan.id}
                className={`relative rounded-3xl p-7 flex flex-col transition-all ${plan.popular ? 'bg-indigo-600 text-white shadow-2xl shadow-indigo-200 scale-105' : 'bg-white border border-gray-200'}`}>
                ...
                <div className="mb-6">
                  <div className={`text-sm font-bold mb-2 ${plan.popular ? 'text-indigo-200' : 'text-gray-500'}`}>{plan.name}</div>
                  <div className="flex items-end gap-1 mb-1">
                    <span className={`text-4xl font-black ${plan.popular ? 'text-white' : 'text-gray-900'}`}>
                      {price === 0 ? 'Free' : `$${price}`}
                    </span>
                    {price > 0 && <span className={`text-sm mb-1.5 ${plan.popular ? 'text-indigo-300' : 'text-gray-400'}`}>/mo</span>}
                  </div>
                ...
                <button
                  onClick={() => plan.id === 'free' ? router.push('/login') : handleUpgrade(plan.id)}
